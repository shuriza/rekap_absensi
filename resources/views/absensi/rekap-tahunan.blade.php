...
<div class="min-h-screen flex flex-col px-6 py-4">
  ...
  <form method="GET" class="flex flex-wrap items-end gap-4 mb-6">
    ...
    <div>
      ...
      <select name="sort"
              class="mt-1 w-56 rounded border-gray-300 text-sm"
              onchange="this.form.submit()">
        <option value=""           {{ request('sort') == '' ? 'selected' : '' }}>— Tidak diurut —</option>
        <option value="nama_asc"   {{ request('sort') == 'nama_asc'   ? 'selected' : '' }}>Nama A → Z</option>
        <option value="nama_desc"  {{ request('sort') == 'nama_desc'  ? 'selected' : '' }}>Nama Z → A</option>
        <option value="total_desc" {{ request('sort') == 'total_desc' ? 'selected' : '' }}>Akumulasi ⇣ Terbanyak</option>
        <option value="total_asc"  {{ request('sort') == 'total_asc'  ? 'selected' : '' }}>Akumulasi ⇡ Tersedikit</option>
      </select>
    </div>
  </form>
  ...
  <div class="overflow-x-auto border border-gray-300 rounded">
    <table id="tabel-rekap"
           class="min-w-full text-sm text-center border-collapse display nowrap">
